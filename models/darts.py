...
class Identity_act(nn.Module):
    def __init__(self):
        super(Identity_act, self).__init__()
...
class Cell(nn.Module):
  ...
  def _compile(self, C, op_names, indices, concat, reduction):
    ...
    self._ops = nn.ModuleList().to(device)
  ...
  def _create_mask(self, x, out):
    print("masks created")
    self.mask = x.new_zeros(out.shape[1])
...
class NetworkCIFAR(nn.Module):
  ...
  def forward(self, input):
    self.input_spatial_dims = input.size()
    logits_aux = None
    activations = []
    prev_activation = 0
    grown = False
    ...
    for i, cell in enumerate(self.cells):
      ...
      _, _, _, s1_finalshape = np.shape(s1)
      if prev_activation != s1_finalshape and len(activations) < 3:
        activations.append(s1)
      prev_activation = s1_finalshape
